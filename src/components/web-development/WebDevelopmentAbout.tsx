...
import { ArrowRight, Gauge, Settings, Shield } from "lucide-react";

const features = [
  { icon: Settings, label: "Custom Development" },
...
function WorldMapGraphic() {
  return (
    <svg className="h-full w-full" viewBox="0 0 440 200" fill="none" aria-hidden="true">
      <defs>
        <radialGradient id="wdNodeGlow" cx="50%" cy="50%" r="50%">
      ...
      {continentDots.map(([cx, cy], i) => (
        <circle key={i} cx={cx} cy={cy} r="2" fill="#ffd700" opacity={0.12 + (i % 5) * 0.04} />
      ))}
      {mapNodes.map((node, i) => (
        <g key={i}>
          <circle cx={node.cx} cy={node.cy} r={node.r * 3} fill="url(#wdNodeGlow)" />
          <circle cx={node.cx} cy={node.cy} r={node.r} fill="#ffd700" opacity={0.95} />
        </g>
      ))}
    </svg>
  );
...
export function WebDevelopmentAbout() {
  return (
        ...
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
          ...
          >
            ...
            <h2 className="mb-6 text-3xl font-bold text-white sm:text-4xl lg:text-5xl">
              ...
              <span className="text-gold">Digital Products.</span>
            </h2>
            <p className="mb-8 max-w-lg text-base leading-relaxed text-muted">
              From landing pages to full-scale SaaS platforms, we craft modern web
              solutions with clean code, great performance, and seamless user
              experiences.
            </p>
            <Link
              href="/#about"
