        ...
        <nav className="hidden items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2 py-1.5 md:flex">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="rounded-full px-5 py-2 text-sm text-muted transition-colors hover:text-white"
            >
              {link.label}
            </a>
          ))}
      ...
      {open && (
        <div className="border-t border-white/5 bg-black px-6 py-4 md:hidden">
          <nav className="flex flex-col gap-2">
            ...
            <Button variant="outline" href="#contact" className="mt-2 justify-center">
              Book a Call
            </Button>
          </nav>
        </div>
